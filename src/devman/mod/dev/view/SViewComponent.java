 ...
 */
public class SViewComponent extends SGridPaneView {

    private SGridFilterPanel moFilterProject;
    private SGridFilterPanel moFilterType;
    
    public SViewComponent(SGuiClient client, String title) {
        super(client, SGridConsts.GRID_PANE_VIEW, SModConsts.DU_CMP, SLibConsts.UNDEFINED, title);
        initComponentCustom();
    }
    ...
    private void initComponentCustom() {
        ...
        moFilterType = new SGridFilterPanel(miClient, this, SModConsts.DS_CMP_TP, SLibConsts.UNDEFINED);
    ...
    public void prepareSqlQuery() {
        ...
        filter = ((SGridFilterValue) moFiltersMap.get(SModConsts.DS_CMP_TP)).getValue();
        if (filter != null && ((int[]) filter).length == 1) {
            sql += (sql.isEmpty() ? "" : "AND ") + "v.fk_cmp_tp = " + ((int[]) filter)[0] + " ";
        }

        msSql = "SELECT "
                + "v.id_prj AS " + SDbConsts.FIELD_ID + "1, "
                + "v.id_cmp AS " + SDbConsts.FIELD_ID + "2, "
                ...
                + "FROM " + SModConsts.TablesMap.get(SModConsts.DU_CMP) + " AS v "
                + "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.PU_PRJ) + " AS vp ON "
                + "v.id_prj = vp.id_prj "
                + "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.DS_CMP_TP) + " AS vt ON "
                + "v.fk_cmp_tp = vt.id_cmp_tp "
                + "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.CU_USR) + " AS ui ON "
                + "v.fk_usr_ins = ui.id_usr "
                + "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.CU_USR) + " AS uu ON "
                + "v.fk_usr_upd = uu.id_usr "
                + (sql.isEmpty() ? "" : "WHERE " + sql)
                + "ORDER BY f_prj, v.name, v.id_prj, v.id_cmp ";
    }
